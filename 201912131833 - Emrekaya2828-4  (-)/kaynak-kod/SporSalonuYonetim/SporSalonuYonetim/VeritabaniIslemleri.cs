    ...
    class VeritabaniIslemleri
    {
        ...
        public bool Yonetici_Ekle(int KullaniciId) // Yeni yonetici ekleme
        {
            try
            {
                ...
                baglanti.Open();
                //veritabanında yapılacak sorgu(örnek: ekleme,silme,güncelleme,arama)
                OleDbCommand komut = new OleDbCommand("Insert Into Yonetici (KullaniciId) values(@KullaniciId)", baglanti);
                //yapılacak sorgu için gerekli parametreler(argüman) girilmesi
                komut.Parameters.AddWithValue("@KullaniciId", KullaniciId);
                //sorgunun başlatılması
                komut.ExecuteNonQuery();
                //veritabanı bağlantısının kapatılması
                baglanti.Close();
                //işlemlerin bitirilmesi
                return true;
            }
            catch
            {
        ...
        public int Yonetici_Kontrol() // Yonetici varligi kontrolu
        {
            try
            {
                ...
                //yönetici kontrolü için veritabanında bulunan yönetici tablosundaki elemanların sayısını alma
        ...
        {
            //buradan sonrasında sürekli işlemler tekrar ettiği için aynı açıklamalar tekrar yapılmamıştır, çünkü yukarıdaki açıklamalar ile aynıdır
            try
            {
                baglanti.Open();
        ...
        public int Kullanici_GetirId(string KullaniciAd) // Kullanici Adından Id sorgulama
        {
            try
            {
                ...
                int Id = 0;
                //veritabanından okuma durumu
                if (oku.Read())
                    //veritabanından okunan bilgi id değişkenine atanır
        ...
        public DataSet Musteri_Listele() // Tum musterileri listeleme
        {
            ...
            {
                baglanti.Open();
                //bir veri seti oluşturulur (boş)
                DataSet dataset = new DataSet();
                //veritabanı sorgusu oluşturulur ve alınan değerler tablo için saklanır
                OleDbDataAdapter adapter = new OleDbDataAdapter("Select *From Musteri", baglanti);
                //veri setinin düzeni müşteri tablosuna göre belirlenir
                adapter.Fill(dataset, "Musteri");
                //veri tutucu kapatılır
                adapter.Dispose();
                //bağlantı sonlandırılır
                baglanti.Close();
                //alınan veriler bir liste halinde geriye döndürülür
                return dataset;
            }
            catch
            {
